 ...
 */
public class BunkerAsyncWorld extends AsyncWorld {

    private static ExecutorService executor = Executors.newCachedThreadPool();

    @Getter
    private BunkerChunkMap chunkMap = new BunkerChunkMap(this);

    //private BunkerWorldServer worldServer;

    public BunkerAsyncWorld() {
    }
    ...
    public BunkerAsyncChunk getChunk(int cx, int cz) {
        return chunkMap.getOrMake(cx, cz);
    }

    protected BunkerAsyncChunk getNewChunk(int cx, int cz) {
        return new BunkerAsyncChunk(this, cx, cz);
    }
    ...
    public void pasteSchematic(Schematic schematic, IntVector3D position, boolean ignoreAir) {
        ...
        for (int x1 = minX; x1 <= maxX; x1++)
            for (int z1 = minZ; z1 <= maxZ; z1++) {
                ...
                int finalX = x1;
                Runnable runnable = () -> {
                    BunkerAsyncChunk chunk = getChunk(finalX, finalZ);
                    int cxi = chunk.getX() << 4;
                    int czi = chunk.getZ() << 4;
    ...
    public void setBlocks(CuboidRegion region, Supplier<Short> blockSupplier) {
        ...
        for (int x = region.getMinimumPoint().getBlockX(); x <= region.getMaximumPoint().getX(); x++) {
            for (int z = region.getMinimumPoint().getBlockZ(); z <= region.getMaximumPoint().getZ(); z++) {
                BunkerAsyncChunk currentChunk = getChunk(x >> 4, z >> 4);
    ...
    public void syncForAllInRegion(CuboidRegion region, BiConsumer<Vector3D, Integer> action, boolean multiThread) {
        ...
        if (threads == 1 || !multiThread) {
            ...
            List<BunkerAsyncChunk> chunks = new ArrayList<>();
            ...
            Iterator<BunkerAsyncChunk> it = chunks.iterator();
            List<Future<Void>> futures = new ArrayList<>();
            while (it.hasNext()) {
                for (int i = 0; i < threads && it.hasNext(); i++) {
                    BunkerAsyncChunk chunk = it.next();
                    chunk.getBukkitChunk().load(true);
     ...
     * not called on creation) see refresh on BunkerAsyncChunk for more info <br>
