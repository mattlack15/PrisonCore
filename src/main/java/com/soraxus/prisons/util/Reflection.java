...
public class Reflection {
    ...
    /**
     * Returns the corresponding Bukkit class, given a CraftBukkit implementation
     * object.
     *
     * @param craftObject A CraftBukkit implementation of a Bukkit class.
     * @return the corresponding Bukkit class, given a CraftBukkit implementation
     * object.
     * @since 1.8
     */
    ...
    /**
     * Uses reflection to execute the named method on the supplied class giving the
     * arguments. Sinks all exceptions, but log an entry and returns
     * <code>null</code>
     *
    ...
    public static Method findMethod(Class<?> aClass, Class returnType, Class... argTypes) throws NoSuchMethodException {
        ...
        if (methods.isEmpty()) {
            throw new NoSuchMethodException(
                    "No method matching " + returnType + " ?(" + Arrays.toString(argTypes) + ")");
        }
        if (methods.size() > 1) {
            throw new NoSuchMethodException(
                    "More than 1 method matching " + returnType + " ?(" + Arrays.toString(argTypes) + ") : " + methods);
        }
        return methods.get(0);
    }

    public static List<Method> findMethods(Class<?> aClass, Class returnType, Class... argTypes)
    ...
    /*
     * public static List<String> dumpMethods(Class aClass) { List<Method> methods =
     * Arrays.asList(aClass.getDeclaredMethods()); List<String> methodDescriptions =
     * new ArrayList<String>(); String version = getNMSVersion(); for (Method m :
     * methods) { List<Class<?>> t0 = Arrays.asList(m.getParameterTypes());
     * List<String> parms = new ArrayList<String>(); for (Class<?> t2 : t0) {
     * parms.add(t2.getDescription()); } // List<String> parms =
     * Arrays.asList(m.getParameterTypes()).stream().map((f) -> (String)
     * f.getDescription()).collect(Collectors.toList()); String parmString =
     * Arrays.toString(parms.toArray(new String[0])); parmString =
     * parmString.substring(1, parmString.length() - 1); String description =
     * (Modifier.isPublic(m.getModifiers()) ? "public " :
     * Modifier.isPrivate(m.getModifiers()) ? "private " : "") +
     * (Modifier.isStatic(m.getModifiers()) ? "static " : "") + m.getReturnType() +
     * " " + m.getDescription() + "(" + parmString + ")"; description =
     * description.replaceAll("class net.minecraft.server." + version + ".", "")
     * .replaceAll("net.minecraft.server." + version + ".",
     * "").replaceAll("java.lang.", ""); methodDescriptions.add(description); }
     * List<String> list = new ArrayList<String>();
     *
     * list.add(aClass.toString().replaceAll("class net.minecraft.server." + version
     * + ".", "") .replaceAll("net.minecraft.server." + version + ".",
     * "").replaceAll("java.lang.", "") + ":");
     * list.addAll(methodDescriptions.stream().sorted().collect(Collectors.toList())
     * ); return list; }
     */
}
